...
func (t *TextParser) Parse_OLD_bugy(text string) {
...
func (t *TextParser) Parse(text string) {
	...
	ws2 := regUsernames.FindAllString(text, -1)
	for _, w := range ws2 {
		//fmt.Println(i, w[1:])
		if len(w) >= 2 {
			userNamesMap[w] += 1
	...
	//TODO: combine functinality below in `for` above
	for k, _ := range tagsMap {
		t.Tags = append(t.Tags, k[1:])
	}

	for k, _ := range userNamesMap {
		t.UserNames = append(t.UserNames, k[1:])
	}
