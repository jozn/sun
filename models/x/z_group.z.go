...
// Group represents a row from 'ms.group'.
...
// GroupByGroupId retrieves a row from 'ms.group' as a Group.
//
// Generated from index 'group_GroupId_pkey'.
func GroupByGroupId(db XODB, groupId int) (*Group, error) {
	var err error
	...
	err = db.QueryRow(sqlstr, groupId).Scan(&g.GroupId, &g.GroupName, &g.MembersCount, &g.GroupPrivacyEnum, &g.CreatorUserId, &g.CreatedTime, &g.UpdatedMs, &g.CurrentSeq)
	if err != nil {
		XOLogErr(err)
		return nil, err
	}
