...
func (c _StoreImpl) OldMsgPush_ByUid(Uid uint) (*OldMsgPush, bool) {
	o, ok := RowCacheIndex.Get("OldMsgPush_Uid:" + fmt.Sprintf("%v", Uid))
	if ok {
	...
	row, err := NewOldMsgPush_Selector().Uid_Eq(Uid).GetRow(base.DB)
	if err == nil {
		RowCacheIndex.Set("OldMsgPush_Uid:"+fmt.Sprintf("%v", row.Uid), row, 0)
		return row, true
	}
...
func (c _StoreImpl) PreLoadOldMsgPush_ByUids(Uids []uint) {
	not_cached := make([]uint, 0, len(Uids))

	for _, id := range Uids {
		_, ok := RowCacheIndex.Get("OldMsgPush_Uid:" + fmt.Sprintf("%v", id))
		if !ok {
			not_cached = append(not_cached, id)
		}
	}
	...
	if len(not_cached) > 0 {
		rows, err := NewOldMsgPush_Selector().Uid_In(not_cached).GetRows(base.DB)
		if err == nil {
			for _, row := range rows {
				RowCacheIndex.Set("OldMsgPush_Uid:"+fmt.Sprintf("%v", row.Uid), row, 0)
			}
...
// DirectMessage2 - direct_message2_MessageId_pkey
