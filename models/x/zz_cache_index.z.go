...
func (c _StoreImpl) DirectLog_ByToUserId(ToUserId int) (*DirectLog, bool) {
	o, ok := RowCacheIndex.Get("DirectLog_ToUserId:" + fmt.Sprintf("%v", ToUserId))
	if ok {
	...
	row, err := NewDirectLog_Selector().ToUserId_Eq(ToUserId).GetRow(base.DB)
	if err == nil {
		RowCacheIndex.Set("DirectLog_ToUserId:"+fmt.Sprintf("%v", row.ToUserId), row, 0)
		return row, true
	}
...
func (c _StoreImpl) PreLoadDirectLog_ByToUserIds(ToUserIds []int) {
	not_cached := make([]int, 0, len(ToUserIds))

	for _, id := range ToUserIds {
		_, ok := RowCacheIndex.Get("DirectLog_ToUserId:" + fmt.Sprintf("%v", id))
		if !ok {
			not_cached = append(not_cached, id)
		}
	}
	...
	if len(not_cached) > 0 {
		rows, err := NewDirectLog_Selector().ToUserId_In(not_cached).GetRows(base.DB)
		if err == nil {
			for _, row := range rows {
				RowCacheIndex.Set("DirectLog_ToUserId:"+fmt.Sprintf("%v", row.ToUserId), row, 0)
			}
