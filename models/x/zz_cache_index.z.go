...
func (c _StoreImpl) MsgPush_ById(Id int) (*MsgPush, bool) {
	o, ok := RowCacheIndex.Get("MsgPush_Id:" + fmt.Sprintf("%v", Id))
	if ok {
	...
	row, err := NewMsgPush_Selector().Id_Eq(Id).GetRow(base.DB)
	if err == nil {
		RowCacheIndex.Set("MsgPush_Id:"+fmt.Sprintf("%v", row.Id), row, 0)
		return row, true
	}
...
func (c _StoreImpl) PreLoadMsgPush_ByIds(Ids []int) {
	not_cached := make([]int, 0, len(Ids))

	for _, id := range Ids {
		_, ok := RowCacheIndex.Get("MsgPush_Id:" + fmt.Sprintf("%v", id))
		if !ok {
			not_cached = append(not_cached, id)
		}
	}
	...
	if len(not_cached) > 0 {
		rows, err := NewMsgPush_Selector().Id_In(not_cached).GetRows(base.DB)
		if err == nil {
			for _, row := range rows {
				RowCacheIndex.Set("MsgPush_Id:"+fmt.Sprintf("%v", row.Id), row, 0)
			}
