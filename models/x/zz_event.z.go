...
//DirectLog2 Events
func OnDirectLog2_AfterInsert(row *DirectLog2) {
	RowCache.Set("DirectLog2:"+strconv.Itoa(row.Id), row, time.Hour*0)
}

func OnDirectLog2_AfterUpdate(row *DirectLog2) {
	RowCache.Set("DirectLog2:"+strconv.Itoa(row.Id), row, time.Hour*0)
}

func OnDirectLog2_AfterDelete(row *DirectLog2) {
	RowCache.Delete("DirectLog2:" + strconv.Itoa(row.Id))
}

func OnDirectLog2_LoadOne(row *DirectLog2) {
	RowCache.Set("DirectLog2:"+strconv.Itoa(row.Id), row, time.Hour*0)
}

func OnDirectLog2_LoadMany(rows []*DirectLog2) {
	for _, row := range rows {
		RowCache.Set("DirectLog2:"+strconv.Itoa(row.Id), row, time.Hour*0)
	}
}
