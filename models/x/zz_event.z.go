...
//DirectMessage2 Events
func OnDirectMessage2_AfterInsert(row *DirectMessage2) {
	RowCache.Set("DirectMessage2:"+strconv.Itoa(row.MessageId), row, time.Hour*0)
}

func OnDirectMessage2_AfterUpdate(row *DirectMessage2) {
	RowCache.Set("DirectMessage2:"+strconv.Itoa(row.MessageId), row, time.Hour*0)
}

func OnDirectMessage2_AfterDelete(row *DirectMessage2) {
	RowCache.Delete("DirectMessage2:" + strconv.Itoa(row.MessageId))
}

func OnDirectMessage2_LoadOne(row *DirectMessage2) {
	RowCache.Set("DirectMessage2:"+strconv.Itoa(row.MessageId), row, time.Hour*0)
}

func OnDirectMessage2_LoadMany(rows []*DirectMessage2) {
	for _, row := range rows {
		RowCache.Set("DirectMessage2:"+strconv.Itoa(row.MessageId), row, time.Hour*0)
	}
}
