...
func OnDirectToMessage_AfterInsert(row *DirectToMessage) {
	RowCache.Set("DirectToMessage:"+strconv.Itoa(row.MessageId), row, time.Hour*0)
}
...
func OnDirectToMessage_AfterDelete(row *DirectToMessage) {
	RowCache.Delete("DirectToMessage:" + strconv.Itoa(row.MessageId))
}
...
func OnUser_AfterInsert(row *User) {
	RowCache.Set("UserView:"+strconv.Itoa(row.Id), row, time.Hour*0)
}
...
func OnUser_AfterDelete(row *User) {
	RowCache.Delete("UserView:" + strconv.Itoa(row.Id))
}
