...
//MsgPush Events
func OnMsgPush_AfterInsert(row *MsgPush) {
	RowCache.Set("MsgPush:"+strconv.Itoa(row.Id), row, time.Hour*0)
}

func OnMsgPush_AfterUpdate(row *MsgPush) {
	RowCache.Set("MsgPush:"+strconv.Itoa(row.Id), row, time.Hour*0)
}

func OnMsgPush_AfterDelete(row *MsgPush) {
	RowCache.Delete("MsgPush:" + strconv.Itoa(row.Id))
}

func OnMsgPush_LoadOne(row *MsgPush) {
	RowCache.Set("MsgPush:"+strconv.Itoa(row.Id), row, time.Hour*0)
}

func OnMsgPush_LoadMany(rows []*MsgPush) {
	for _, row := range rows {
		RowCache.Set("MsgPush:"+strconv.Itoa(row.Id), row, time.Hour*0)
	}
}
...
//MsgPushEvent Events
func OnMsgPushEvent_AfterInsert(row *MsgPushEvent) {
	RowCache.Set("MsgPushEvent:"+strconv.Itoa(row.Id), row, time.Hour*0)
}

func OnMsgPushEvent_AfterUpdate(row *MsgPushEvent) {
	RowCache.Set("MsgPushEvent:"+strconv.Itoa(row.Id), row, time.Hour*0)
}

func OnMsgPushEvent_AfterDelete(row *MsgPushEvent) {
	RowCache.Delete("MsgPushEvent:" + strconv.Itoa(row.Id))
}

func OnMsgPushEvent_LoadOne(row *MsgPushEvent) {
	RowCache.Set("MsgPushEvent:"+strconv.Itoa(row.Id), row, time.Hour*0)
}

func OnMsgPushEvent_LoadMany(rows []*MsgPushEvent) {
	for _, row := range rows {
		RowCache.Set("MsgPushEvent:"+strconv.Itoa(row.Id), row, time.Hour*0)
	}
}
...
//MessageOld Events
func OnMessageOld_AfterInsert(row *MessageOld) {
	RowCache.Set("MessageOld:"+strconv.Itoa(row.Id), row, time.Hour*0)
}

func OnMessageOld_AfterUpdate(row *MessageOld) {
	RowCache.Set("MessageOld:"+strconv.Itoa(row.Id), row, time.Hour*0)
}

func OnMessageOld_AfterDelete(row *MessageOld) {
	RowCache.Delete("MessageOld:" + strconv.Itoa(row.Id))
}

func OnMessageOld_LoadOne(row *MessageOld) {
	RowCache.Set("MessageOld:"+strconv.Itoa(row.Id), row, time.Hour*0)
}

func OnMessageOld_LoadMany(rows []*MessageOld) {
	for _, row := range rows {
		RowCache.Set("MessageOld:"+strconv.Itoa(row.Id), row, time.Hour*0)
	}
}
