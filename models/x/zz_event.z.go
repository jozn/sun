...
//LikesCopy Events
func OnLikesCopy_AfterInsert(row *LikesCopy) {
	RowCache.Set("LikesCopy:"+strconv.Itoa(row.Id), row, time.Hour*0)
}

func OnLikesCopy_AfterUpdate(row *LikesCopy) {
	RowCache.Set("LikesCopy:"+strconv.Itoa(row.Id), row, time.Hour*0)
}

func OnLikesCopy_AfterDelete(row *LikesCopy) {
	RowCache.Delete("LikesCopy:" + strconv.Itoa(row.Id))
}

func OnLikesCopy_LoadOne(row *LikesCopy) {
	RowCache.Set("LikesCopy:"+strconv.Itoa(row.Id), row, time.Hour*0)
}

func OnLikesCopy_LoadMany(rows []*LikesCopy) {
	for _, row := range rows {
		RowCache.Set("LikesCopy:"+strconv.Itoa(row.Id), row, time.Hour*0)
	}
}
...
func OnPostCopy_AfterInsert(row *PostCopy) {
	RowCache.Set("PostCopy:"+strconv.Itoa(row.Id), row, time.Hour*0)
}
...
func OnPostCopy_AfterDelete(row *PostCopy) {
	RowCache.Delete("PostCopy:" + strconv.Itoa(row.Id))
}
