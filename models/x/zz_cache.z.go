...
func (c _StoreImpl) GetMsgPushEventById(Id int) (*MsgPushEvent, bool) {
	o, ok := RowCache.Get("MsgPushEvent:" + strconv.Itoa(Id))
	if ok {
		if obj, ok := o.(*MsgPushEvent); ok {
			return obj, true
		}
	}
	obj2, err := MsgPushEventById(base.DB, Id)
	if err == nil {
		return obj2, true
	}
	XOLogErr(err)
...
func (c _StoreImpl) PreLoadMsgPushEventByIds(ids []int) {
	not_cached := make([]int, 0, len(ids))

	for _, id := range ids {
		_, ok := RowCache.Get("MsgPushEvent:" + strconv.Itoa(id))
		if !ok {
			not_cached = append(not_cached, id)
		}
	}
	...
	if len(not_cached) > 0 {
		NewMsgPushEvent_Selector().Id_In(not_cached).GetRows(base.DB)
	}
}
...
func (c _StoreImpl) GetMessageOldById(Id int) (*MessageOld, bool) {
	o, ok := RowCache.Get("MessageOld:" + strconv.Itoa(Id))
	if ok {
		if obj, ok := o.(*MessageOld); ok {
			return obj, true
		}
	}
	obj2, err := MessageOldById(base.DB, Id)
	if err == nil {
		return obj2, true
	}
	XOLogErr(err)
...
func (c _StoreImpl) PreLoadMessageOldByIds(ids []int) {
	not_cached := make([]int, 0, len(ids))

	for _, id := range ids {
		_, ok := RowCache.Get("MessageOld:" + strconv.Itoa(id))
		if !ok {
			not_cached = append(not_cached, id)
		}
	}
	...
	if len(not_cached) > 0 {
		NewMessageOld_Selector().Id_In(not_cached).GetRows(base.DB)
	}
}

func (c _StoreImpl) GetMsgPushById(Id int) (*MsgPush, bool) {
	o, ok := RowCache.Get("MsgPush:" + strconv.Itoa(Id))
	if ok {
		if obj, ok := o.(*MsgPush); ok {
			return obj, true
		}
	}
	obj2, err := MsgPushById(base.DB, Id)
	if err == nil {
		return obj2, true
	}
	XOLogErr(err)
...
func (c _StoreImpl) PreLoadMsgPushByIds(ids []int) {
	not_cached := make([]int, 0, len(ids))

	for _, id := range ids {
		_, ok := RowCache.Get("MsgPush:" + strconv.Itoa(id))
		if !ok {
			not_cached = append(not_cached, id)
		}
	}
	...
	if len(not_cached) > 0 {
		NewMsgPush_Selector().Id_In(not_cached).GetRows(base.DB)
	}
}
