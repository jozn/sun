...
func (c _StoreImpl) GetDirectMessage2ByMessageId(MessageId int) (*DirectMessage2, bool) {
	o, ok := RowCache.Get("DirectMessage2:" + strconv.Itoa(MessageId))
	if ok {
		if obj, ok := o.(*DirectMessage2); ok {
			return obj, true
		}
	}
	obj2, err := DirectMessage2ByMessageId(base.DB, MessageId)
	if err == nil {
		return obj2, true
	}
	XOLogErr(err)
...
func (c _StoreImpl) PreLoadDirectMessage2ByMessageIds(ids []int) {
	not_cached := make([]int, 0, len(ids))

	for _, id := range ids {
		_, ok := RowCache.Get("DirectMessage2:" + strconv.Itoa(id))
		if !ok {
			not_cached = append(not_cached, id)
		}
	}
	...
	if len(not_cached) > 0 {
		NewDirectMessage2_Selector().MessageId_In(not_cached).GetRows(base.DB)
	}
}
