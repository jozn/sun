...
func (c _StoreImpl) GetDirectLog2ById(Id int) (*DirectLog2, bool) {
	o, ok := RowCache.Get("DirectLog2:" + strconv.Itoa(Id))
	if ok {
		if obj, ok := o.(*DirectLog2); ok {
			return obj, true
		}
	}
	obj2, err := DirectLog2ById(base.DB, Id)
	if err == nil {
		return obj2, true
	}
	XOLogErr(err)
...
func (c _StoreImpl) PreLoadDirectLog2ByIds(ids []int) {
	not_cached := make([]int, 0, len(ids))

	for _, id := range ids {
		_, ok := RowCache.Get("DirectLog2:" + strconv.Itoa(id))
		if !ok {
			not_cached = append(not_cached, id)
		}
	}
	...
	if len(not_cached) > 0 {
		NewDirectLog2_Selector().Id_In(not_cached).GetRows(base.DB)
	}
}
