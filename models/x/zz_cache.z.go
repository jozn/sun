...
func (c _StoreImpl) GetChatByChatKey(ChatKey int) (*Chat, bool) {
	o, ok := RowCache.Get("Chat:" + strconv.Itoa(ChatKey))
	if ok {
		if obj, ok := o.(*Chat); ok {
			return obj, true
		}
...
func (c _StoreImpl) PreLoadChatByChatKeys(ids []int) {
	not_cached := make([]int, 0, len(ids))
...
func (c _StoreImpl) GetDirectUpdateById(Id int) (*DirectUpdate, bool) {
	...
	obj2, err := DirectUpdateById(base.DB, Id)
	if err == nil {
		return obj2, true
	}
	XOLogErr(err)
...
func (c _StoreImpl) PreLoadDirectUpdateByIds(ids []int) {
	not_cached := make([]int, 0, len(ids))
	...
	if len(not_cached) > 0 {
		NewDirectUpdate_Selector().Id_In(not_cached).GetRows(base.DB)
	}
}
