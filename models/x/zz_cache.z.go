...
func (c _StoreImpl) GetMsgFileById(Id int) (*MsgFile, bool) {
	o, ok := RowCache.Get("MsgFile:" + strconv.Itoa(Id))
	if ok {
		if obj, ok := o.(*MsgFile); ok {
			return obj, true
		}
	}
	obj2, err := MsgFileById(base.DB, Id)
	if err == nil {
		return obj2, true
	}
	XOLogErr(err)
...
func (c _StoreImpl) PreLoadMsgFileByIds(ids []int) {
	not_cached := make([]int, 0, len(ids))

	for _, id := range ids {
		_, ok := RowCache.Get("MsgFile:" + strconv.Itoa(id))
		if !ok {
			not_cached = append(not_cached, id)
		}
	}
	...
	if len(not_cached) > 0 {
		NewMsgFile_Selector().Id_In(not_cached).GetRows(base.DB)
	}
}
