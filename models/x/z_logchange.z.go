...
// LogChange represents a row from 'ms.log_changes'.
...
// LogChangeById retrieves a row from 'ms.log_changes' as a LogChange.
//
// Generated from index 'log_changes_Id_pkey'.
func LogChangeById(db XODB, id int) (*LogChange, error) {
	var err error
	...
	err = db.QueryRow(sqlstr, id).Scan(&lc.Id, &lc.T)
	if err != nil {
		XOLogErr(err)
		return nil, err
	}
