...
/// IMPORTANT BY ME:
// this is a package from copyied github.com/drone/routes
//only on changelog to this file:
// at line ~195 we have replaced
// r.URL.RawQuery = url.Values(values).Encode() + "&" + r.URL.RawQuery
//with:=>  r.URL.RawQuery = url.Values(values).Encode()
//it caused duplicated params in Req.From response
...
func NewPrefix(prefix string) *RouteMux {
	return &RouteMux{prefix: prefix}
}
...
func (m *RouteMux) AddRoute(method string, pattern string, handler http.HandlerFunc) {
	...
	pattern = m.prefix + pattern //BY ME
